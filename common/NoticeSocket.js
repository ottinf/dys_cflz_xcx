...
export default class NoticeSocket {
  ...
  startTask() {
    ...
    this.timer = setInterval(async () => {
      if (!this.token) {
        ...
        if (sockettoken && sockettoken.notice)
          this.token = sockettoken.notice
        else {
          let userinfo = uni.getStorageSync('userInfo');
          if (userinfo && userinfo.SocketToken && userinfo.SocketToken.notice)
            this.token = userinfo.SocketToken.notice
        }
        if (!this.token)
          return;
      }
      if (!this.url) {
        ...
        if (baseurl == 'https://api.d1she.cn') {
          this.url = `wss://api2.d1she.cn/socket/notice?token=${this.token}`
        } else {
          baseurl = baseurl.replace('http', 'ws')
          this.url = `${baseurl}/socket/notice?token=${this.token}`
        }
        return;
      }
      if (!this.is_connect()) {
